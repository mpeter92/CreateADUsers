...
for /f "tokens=1,2 delims= " %%G in (c:\temp\users.txt) do echo "CN=%%G %%H,OU=users,DC=%_domain%,DC=%_domainroot%",user,%%G%%H,%%H,%%G,%%G.%%H@%_domain%.%_domainroot% >> c:\temp\users.csv


::import users to active directory

csvde -i c:\temp\users.csv


::
::for /f "tokens=*" %%G IN ('dsquery user "OU=users,DC=%_domain%,DC=%_domainroot%"') do dsmod user %%G -pwd password -disabled No